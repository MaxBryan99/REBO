...
{
    public partial class FrmValidaNCredito : Form
    {
        ...
        private string valIdNc = "";
        private ClsNotCre ObjNotCre = new ClsNotCre();

        public FrmValidaNCredito()
        {
            InitializeComponent();
            valIdNc = FrmNotaCredito.nIdNc.ToString();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (datos.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    if (MessageBox.Show("¿Está seguro de querer ANULAR el registro de Nota de Crédito?", "SISTEMA", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No)
                    {
                        return;
                    }

                    if (ObjNotCre.Anular(valIdNc, codAlmacen, rucEmpresa, textBox1.Text.ToString().Trim()))
                    {
                ...
                {
                    if (MessageBox.Show("¿Está seguro de querer ELIMINAR el registro de Nota de Crédito?", "SISTEMA", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No)
                    {
                        return;
                    }
                    if (ObjNotCre.Eliminar(valIdNc, codAlmacen, rucEmpresa, textBox1.Text.ToString().Trim()))
                    {
