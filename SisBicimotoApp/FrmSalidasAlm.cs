...
{
    public partial class FrmSalidasAlm : Form, IResponsable, ISalida
    {
        public ISalida Opener { get; set; }

        public static char nmSal = 'N';
        ...
        public void CargarConsulta(string vValor)
        {
            if (vValor.ToString().Equals("V"))
            {
                ...
                datos = csql.dataset("Call SpSalidaConsulta('" + vFecha1.ToString() + "','" + vFecha2.ToString() + "','" + vResp.ToString() + "','" + vTDoc.ToString().Trim() + "','" + vAlmacen.ToString().Trim() + "','" + rucEmpresa.ToString().Trim() + "')");
                Grid1.DataSource = datos.Tables[0];
                Grilla();
            }
        }

        #endregion ISalida Members
        ...
        {
            nmSal = 'N';
            FrmIngresosAlm.nmIng = 'N';
            FrmAddSalida frmAddSalida = new FrmAddSalida();
            frmAddSalida.WindowState = FormWindowState.Normal;
            frmAddSalida.ShowDialog(this);
        }

        private void FrmSalidasAlm_Load(object sender, EventArgs e)
        {
            //Carga Almacen
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                nId = Grid1.CurrentRow.Cells[6].Value.ToString();
                FrmValidaSalida frmValidaSalida = new FrmValidaSalida();
                frmValidaSalida.WindowState = FormWindowState.Normal;
                frmValidaSalida.Opener = this;
                //frmAddCompra.MdiParent = this.MdiParent;
                //frmAddCompra.Show();
                frmValidaSalida.ShowDialog(this);
            }
            else
            {
                MessageBox.Show("Seleccione un registro", "SISTEMA");
