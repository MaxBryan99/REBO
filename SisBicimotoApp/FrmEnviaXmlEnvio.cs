...
{
    public partial class FrmEnviaXmlEnvio : Form, IEnvio
    {
        ...
        private void FrmEnviaXmlEnvio_Load(object sender, EventArgs e)
        {
            string nIdVenta = "";
            string nomXml = "";
            string vDoc = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ObjVenta.BuscarVenta(label4.Text.ToString(), rucEmpresa, FrmEnviarSunat.vAlm))
                {
                ...
                if (ObjVenta.ArchivoXml.Length > 0)
                {
                    ...
                    if (ObjVenta1.BuscarVenta(label4.Text.ToString(), rucEmpresa, FrmEnviarSunat.vAlm))
                    {
                        //MessageBox.Show("Error no se encontró datos de Venta, VERIFIQUE!!!", "SISTEMA");
                        //return;
                    }
                    ...
                    ObjGrabaXML.enviarXML(TipoDocumento, label4.Text.ToString(), codDoc, ObjVenta.Serie, ObjVenta.Numero, textBox3.Text.ToString(), RutaArchivo, nomXml, rucEmpresa.ToString(), FrmEnviarSunat.vAlm, vUsuario);
                }
                else
                {
                    ObjGrabaXML.generaXMLFactura(label4.Text.ToString(), rucEmpresa, FrmEnviarSunat.vAlm, false);
                    ...
                    ObjGrabaXML.objEnvio = this;
                    ObjGrabaXML.enviarXML(TipoDocumento, label4.Text.ToString(), codDoc, ObjVenta.Serie, ObjVenta.Numero, textBox3.Text.ToString(), RutaArchivo, ObjVenta1.ArchivoXml, rucEmpresa.ToString(), FrmEnviarSunat.vAlm, vUsuario);
                }
